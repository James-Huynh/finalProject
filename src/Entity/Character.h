...
private:
	...
	int level, xp;
	bool alive;		// Tracks if the character is alive
	string charName, charDescription;
	Inventory myInventory;
	Equipment myEquipment;
	int money;
	double computeDamageDealt();
	double computeDamageReceived(double dmgIn);
	double computeTotalAttack();					// Helper function that computes the total attacking damage
	double computeTotalDefense(); 				// Helper function that computes the final damage receives
...
public:
	Character(string name = "TestName",  char roleId = '1', string description = "None");
	...
	bool pickUpItem(Item* theItem);	// Adds the item to inventory
